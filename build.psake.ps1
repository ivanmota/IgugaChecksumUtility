...
Task Test -depends Build -requiredVariables TestRootDir, ModuleName, CodeCoverageEnabled, CodeCoverageFiles {
    ...
    try {
        ...
        $pesterConfig.Run.Path = $TestRootDir
        ...
        if ($TestOutputFile) {
            $pesterConfig.TestResult.OutputPath = $TestOutputFile
            $pesterConfig.TestResult.OutputFormat = $TestOutputFormat
        }

        # To control the Pester code coverage, a boolean $CodeCoverageEnabled is used.
        if ($CodeCoverageEnabled) {
            $pesterConfig.CodeCoverage.Enabled = $CodeCoverageEnabled
            $pesterConfig.CodeCoverage.Path = (Get-ChildItem $CodeCoverageFiles -Recurse).FullName;
        }
        ...
        # if ($CodeCoverageEnabled) {
        #     "Pester code coverage on specified files: ${testResult.CodeCoverage}"
        # }
    }
    finally {
